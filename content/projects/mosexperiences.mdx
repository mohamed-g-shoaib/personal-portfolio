...
The current release candidate passes its pre-deployment SEO checks. Search Console inspection, an external crawl, and real-user performance data will only be available after deployment, so local checks are not presented as proof of search performance.
